...
class Solution(object):
    ...
    def get_valid_antinodes(
        ...
        valid_antinodes = set()
        if is_valid(pair[0] + d_np_r, pair[1] + d_np_c):
            valid_antinodes.add((pair[0] + d_np_r, pair[1] + d_np_c))

        if is_valid(node[0] - d_np_r, node[1] - d_np_c):
            valid_antinodes.add((node[0] - d_np_r, node[1] - d_np_c))
