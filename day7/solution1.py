...
class Solution(object):
    ...
    def can_reach_target(
        ...
        """Adds/multiplies curr_val with nums to return if target is reachable.
        
        Recursively either adds or multiplies the num at given `idx` in `nums`
        until all numbers have been processed, in order.
        """
        if idx == len(nums):
            if curr_val == target:
                return True
        ...
        return self.can_reach_target(
            nums, idx + 1, curr_val + nums[idx], target
